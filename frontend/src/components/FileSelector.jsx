    ...
    return (
        <div className="modal-overlay">
            <div className="modal-content" style={{ maxWidth: '800px', textAlign: 'left', maxHeight: '90vh', overflowY: 'auto' }}>
                <h2 style={{ marginTop: 0, color: '#202124' }}>Select Files to Download</h2>
                ...
                {Object.entries(groupedFiles).map(([type, groupFiles]) => {
                    ...
                    return (
                        <div key={type} style={{ marginBottom: '20px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', backgroundColor: '#f1f3f4', padding: '8px', borderRadius: '4px' }}>
                                <input
                                    type="checkbox"
                                    checked={allSelected}
                                ...
                                />
                                <h4 style={{ margin: 0, color: '#202124' }}>{type} ({groupFiles.length})</h4>
                            </div>
                            <div style={{ paddingLeft: '20px', marginTop: '8px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                                {groupFiles.map(f => (
                                    <div key={f.id} style={{ display: 'flex', alignItems: 'center', fontSize: '0.9rem', color: '#3c4043' }}>
                                        <input
                                            type="checkbox"
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '20px', sticky: 'bottom' }}>
                    <button onClick={onCancel} style={{ color: '#202124', padding: '8px 16px', background: 'transparent', border: '1px solid #ccc', borderRadius: '4px', cursor: 'pointer' }}>
                        Cancel
                    </button>
                    <button
